...
# %% Voy a meter el resto en una función para ejecutarlo desde tarea_1.py
def execute(df: pd.DataFrame):
    ...
    # %%
    df_top5_speakers = df_with_states[df_with_states["speaker"].isin(top5_speakers)].copy()
    speeches_by_state = (
    ...
    color_map = {"Partido Demócrata": "#1f77b4", "Partido Republicano": "red", "Empate": "gray"}
    ...
    ax = (
        df_top5_speakers_news_channel
        .groupby(["news_channel", "candidate_affiliation"])
        .size()
        .unstack()
        .fillna(0)
        .plot(kind="bar", stacked=True, color=list(color_map.values()), ax=ax)
    )
    fig.suptitle(t='Distribución de Discursos por Partido y Canal de Noticias', fontweight='bold')
